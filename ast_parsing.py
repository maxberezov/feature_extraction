...
def process_loops(loop_list):
    ...
    for loop, label_name, level in loop_list:
        ...
        features[loop] = {'label': label_name, 'statements': 0, 'writes': 0, 'reads': 0,
                          ...
                          'level': level, 'iterators': set(), 'var_and_iter': 0,
    ...
    for value in features.values():
        ...
        var_and_iter = value['iterators'].intersection(value['vars'])
...
def process_array_ref(array_ref, features, loop):
    if array_ref.__class__ is ID:
        features[loop]['arrays'].add(array_ref.name)
    else:
        process_array_ref(array_ref.name, features, loop)
        process_subscript(array_ref, features, loop)


def process_binary_op_for_subscripts(bin_op, features, loop):
    if bin_op.left.__class__ is ID:
        features[loop]['iterators'].add(bin_op.left.name)
    elif bin_op.left.__class__ is BinaryOp:
        process_binary_op_for_subscripts(bin_op.left, features, loop)

    if bin_op.right.__class__ is ID:
        features[loop]['iterators'].add(bin_op.right.name)
    elif bin_op.right.__class__ is BinaryOp:
        process_binary_op_for_subscripts(bin_op.right, features, loop)


def process_subscript(array_ref, features, loop):
    subscript = array_ref.subscript
    if subscript.__class__ is BinaryOp:
        process_binary_op_for_subscripts(subscript, features, loop)
    else:
        if hasattr(subscript, 'name'):
            name = subscript.name
            features[loop]['iterators'].add(name)
...
def process_lvalue(lval, features, loop):
    if lval.__class__ is ID:
        ...
        if lval.__class__ is ArrayRef:
            process_array_ref(lval, features, loop)
            features[loop]['writes'] += 1
    ...
    if rval.__class__ is BinaryOp:
        ...
        if rval.__class__ is ID:
            ...
            if rval.__class__ is ArrayRef:
                process_array_ref(rval, features, loop)
                features[loop]['reads'] += 1
